...
[![PR checks](https://github.com/unhumanplane/astrbot_plugin_auto_trpg_dm/actions/workflows/pr-check.yml/badge.svg)](https://github.com/unhumanplane/astrbot_plugin_auto_trpg_dm/actions/workflows/pr-check.yml)
...
```text
/dm 我想开一个黑暗奇幻调查团，背景是一座被旧神梦境污染的港城。请先帮我开场。
```

创建或补充角色卡：

```text
/dm 创建角色卡：名字林鸦，身份是人类游荡者，风格谨慎寡言，擅长潜行、开锁和短弓射击，装备有短弓、匕首、皮甲，弱点是不信任贵族。
```

声明角色行动：

```text
/dm 我贴着右侧石墙潜行到木箱后，观察最近敌人的位置。如果射线不被遮挡，就用短弓攻击。
```
...
/dm 重开当前团
/dm 确认重开 RESET-XXXX
/dm 开新团：废土科幻逃亡，玩家从一辆失控装甲列车上醒来。
```

`RESET-XXXX` 是插件返回的二次确认码；只有确认后才会清空当前团存档。
